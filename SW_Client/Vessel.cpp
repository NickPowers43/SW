...
namespace SW_Client
{
	...
	void Vessel::ReadSetChunkMessage(NetworkReader* nr, NetworkWriter* nw)
	{
		...
		//cout << "Received " << chunkCount << " chunks from server";

		std::vector<TileChunk*> newChunks;
		...
		while (newChunks.size() > 0) {
			...
			if (swChunk)
			{
				SW_Client::TileChunk* existingChunk = static_cast<SW_Client::TileChunk*>(swChunk);
			...
			currentVessel->tiles.SetChunk(static_cast<SW::TileChunk*>(chunk));
